...
// donorId is optional and auto-generated by backend when omitted.
...
export default function RegisterDonorPage() {
  ...
  const handleSubmit = async () => {
    if (!form.userCert || !form.name || !form.email) {
      toast("UserCert, name, and email are required", "error");
      return;
    }
    setLoading(true);
    try {
      const payload = {
        ...form,
        donorId: String(form.donorId || "").trim(),
      };
      if (!payload.donorId) {
        delete payload.donorId;
      }

      const response = await donorApi.register(payload);
      const generatedId = response?.data?.generatedId || response?.data?.data?.donorId;
      toast(`Donor registered on ledger${generatedId ? `: ${generatedId}` : ""}`, "success");
      ...
      <div className="grid-2">
        <div className="card">
          <div className="form-grid section-gap">
            ...
            <div className="form-group"><label>Donor ID (optional)</label><input value={form.donorId} onChange={upd("donorId")} placeholder="Leave empty for auto ID" /></div>
