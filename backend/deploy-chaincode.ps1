...
# Step 1: Rebuild chaincode Docker image every deploy (avoid stale code)
Write-Host "[1/3] Rebuilding chaincode Docker image..." -ForegroundColor Yellow
wsl -d Ubuntu bash -c 'docker rmi ccc01_ccaas_image:latest 2>/dev/null; true'
wsl -d Ubuntu bash -c "cd $wslRepoRoot/fabric-samples/test-network; docker build --no-cache -f ../asset-transfer-basic/chaincode-javascript/Dockerfile -t ccc01_ccaas_image:latest --build-arg CC_SERVER_PORT=9999 ../asset-transfer-basic/chaincode-javascript"
...
wsl -d Ubuntu bash -c "cd $wslRepoRoot/fabric-samples/test-network; ./network.sh deployCCAAS -ccn ccc01 -ccv 1.0 -ccs 1 -ccp ../asset-transfer-basic/chaincode-javascript"
